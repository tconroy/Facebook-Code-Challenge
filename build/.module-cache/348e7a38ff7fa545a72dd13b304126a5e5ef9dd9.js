...
var Calendar = React.createClass({displayName: "Calendar",
  ...
  detectSiblings: function( oldEventArr ) {
    ...
    for( var i=0; i<n; i++ ) {
      ...
      console.log( 'event['+i+'] has sibIndex of ' + currObj.sibIndex + ' and ' + currObj.siblings + ' siblings' );
  ...
  render: function() {
    ...
    return (
      ...
      calEvents.map(function(item, i){
          return(React.createElement(Event, {key: i, siblings: item.siblings, sibIndex: item.sibIndex, top: item.start, height: item.end - item.start}));
      })
      )
    );
  }
...
var Event = React.createClass({displayName: "Event",
  render: function() {
    ...
    if ( sibIndex === -1 ) {
      ...
      width = 100 * (sibIndex / siblings);
      left = width * (siblings * sibIndex);
    }

    /*
      if index is -1, then the event has no siblings and can have full width.

      if the index is 0, the event is the first slot and its width is 100 / number of siblings

      if the index is >0, the event is in

    */

    // kinda working
    //width = ( siblings === 0 ? 100 : 50*siblings / siblings );
    // left = ( siblings >= 2 ? 100/siblings : 0 );

    // width = 100.0 / (siblings+1); // total # siblings
    // left = width * (sibIndex); // the index of this sibling
    // console.log('sibIndex is ' + sibIndex);

    // console.log( 'width = ' + width + ' with ' + siblings + ' siblings' );
    // console.log( 'the left is ' + left + ' with ' + siblings + ' siblings.' );
