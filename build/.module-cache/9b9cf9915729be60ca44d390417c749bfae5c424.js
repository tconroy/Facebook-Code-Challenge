  ...
  render: function() {
    ...
    return (
      ...
      calEvents.map(function(item, i){
          var width = (100/item.siblings || 100),
              left = (width * item.siblings);
          console.log( item.siblings );
          return(React.createElement(Event, {key: i, siblings: item.siblings, top: item.start, left: left, width: width, height: item.end - item.start}));
      })
      )
    );
  }
