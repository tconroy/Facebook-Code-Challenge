...
var Calendar = React.createClass({displayName: "Calendar",
  ...
  formatSlots: function( oldEventArr ) {
    ...
    for( var i=0; i<n; i++ ) {
      ...
      currObj.totalSlots = 0; currObj.slotIndex = 0;
      for( var x=0; x<n; x++ ) {
    ...
    // newEventArr[0].totalSlots = 1;
    // newEventArr[1].totalSlots = 2;
    // newEventArr[2].totalSlots = 2;
    // newEventArr[3].totalSlots = 2;
  },
  ...
  render: function() {
    ...
    return (
      ...
      calEvents.map(function(item, i){
          return(React.createElement(Event, {key: i, totaSlots: item.totalSlots, slotIndex: item.slotIndex, top: item.start, height: item.end - item.start}));
      })
      )
    );
  }
